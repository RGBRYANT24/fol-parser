...
namespace LogicSystem
{
    ...
    class ColoringNeuralTest : public ::testing::Test
    {
    protected:
        void SetUp() override
        {
    ...
    TEST_F(ColoringNeuralTest, K3GraphCannotBeTwoColoredMCTS)
    {
        // 1. 相邻顶点不能同色（红色）
        // ¬E(x,y)∨ ¬R(x)∨ ¬R(y)∨ uncol
    ...
    TEST_F(ColoringNeuralTest, K4GraphCannotBeTwoColoredMCTS)
    {
        // 1. 相邻顶点不能同色（红色）
        // ¬E(x,y)∨ ¬R(x)∨ ¬R(y)∨ uncol
        Clause adjacentNotSameColorR;
        ...
        // 打印知识库内容
        std::cout << "K4 Two-Coloring Knowledge Base:" << std::endl;
        kb.print();
    ...
    TEST_F(ColoringNeuralTest, K4GraphCannotBeThreeColoredMCTS)
    {
        // 1. 相邻顶点不能同色（红色）
        // ¬E(x,y)∨ ¬R(x)∨ ¬R(y)∨ uncol
        Clause adjacentNotSameColorR;
        ...
        // 验证结果
        EXPECT_TRUE(proved);
        std::cout << "SLI Resolution with MCTS proves that K4 graph cannot be two-colored." << std::endl;
    }
