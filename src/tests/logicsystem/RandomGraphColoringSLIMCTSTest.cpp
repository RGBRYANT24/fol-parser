...
namespace LogicSystem
{
    ...
    protected:
        void SetUp() override
        {
            ...
            // 添加常量（图的顶点）- 预先添加所有可能用到的顶点
            const_a = kb.addConstant("a");
            const_b = kb.addConstant("b");
            const_c = kb.addConstant("c");
            const_d = kb.addConstant("d");
            const_e = kb.addConstant("e");
            const_f = kb.addConstant("f");
            const_g = kb.addConstant("g");
            const_h = kb.addConstant("h");
            const_i = kb.addConstant("i");
            const_j = kb.addConstant("j");
        ...
        std::vector<std::pair<SymbolId, SymbolId>> generateUniqueRandomGraph(
            int numNodes, std::unordered_set<std::string> &generatedGraphs)
        {
        ...
        // 将图结构保存到文件
        void saveGraphToFile(const std::vector<std::pair<SymbolId, SymbolId>> &edges, int numNodes, const std::string &filename)
        {
    ...
    TEST_F(RandomGraphColoringTest, RandomGraphTwoColoringMCTS)
    {
        ...
        for (int test = 0; test < numTests; test++)
        {
            ...
            // 保存图结构到文件
            // saveGraphToFile(edges, numNodes, filename);

            // 设置两染色问题的知识库并添加图的边
            setupTwoColoringKB();
            addGraphEdgesToKB(edges);
