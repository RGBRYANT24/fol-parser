...
namespace LogicSystem
{
    ...
    protected:
        ...
        void loadGraphFromJson(const std::string& filePath) 
        {
            ...
            std::map<std::string, std::string> constMapping = {
                {"CONST0", "a"}, {"CONST1", "b"}, {"CONST2", "c"}, {"CONST3", "d"}, {"CONST4", "e"},
                {"CONST5", "f"}, {"CONST6", "g"}, {"CONST7", "h"}, {"CONST8", "i"}, {"CONST9", "j"}
            };
    ...
    TEST_F(GraphColoringTest, BatchTestTwoColoring)
    {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(dataDir)) {
            if (entry.path().extension() == ".json") {
                ...
                // 打印知识库内容
                std::cout << "Knowledge Base for " << filename << ":" << std::endl;
                kb.print();
                ...
                // bool proved = prover.prove("results/" + filename);
                
                // // 输出结果但不断言 - 有些图可能是2着色的
                // std::cout << "Graph " << filename << (proved ? " cannot" : " can") 
    ...
    TEST_F(GraphColoringTest, TestSpecificGraphThreeColoring)
    {
        ...
        // 执行SLI消解
        SLIHeuristicProver prover(kb, goal);
        bool proved = prover.prove("results/specific_graph_3coloring");
    ...
    TEST_F(GraphColoringTest, PetersenGraphThreeColoring)
    {
        ...
        std::vector<std::pair<SymbolId, SymbolId>> edges = {
            {const_a, const_b}, {const_b, const_c}, {const_c, const_d}, {const_d, const_e}, {const_e, const_a},
            {const_f, const_h}, {const_g, const_i}, {const_h, const_j}, {const_i, const_f}, {const_j, const_g},
            {const_a, const_f}, {const_b, const_g}, {const_c, const_h}, {const_d, const_i}, {const_e, const_j}
        };
        ...
        // 执行SLI消解
        SLIHeuristicProver prover(kb, goal);
        bool proved = prover.prove("results/petersen_3coloring");
